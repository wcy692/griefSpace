...
const getThisJournal = () => {
  ...
  xhr.onload = function() {
    if (this.status == 200) {
      ...
      allQuestionDisplay.forEach((display, i) => display.innerText = res[2][i][0].toUpperCase() + res[2][i].substring(1).trim());

      const allTextArea = document.querySelectorAll('.questionSlide textarea[name=ansText]');
      allTextArea.forEach((area, i) => area.innerHTML = res[3][i]);
      ...
      questionSlides.forEach((slide, i) => {
        if (questionSlides[i+1] != null) {
          ...
          let newDiv = document.createElement("div");
          newDiv.setAttribute("class", "bottomBtns flex alignBottom justifyBetween");
          newDiv.innerHTML = appendSaveBtn();
          slide.appendChild(newDiv);
        }
        // questionSlides[i+1] != null ? slide.innerHTML += appendNextBtn() : slide.innerHTML += appendSaveBtn();
      });
      ...
      allEmotionDesc.forEach((emotion, i) => {
        if (emotion.innerText.trim().toLowerCase() == res[1].trim().toLowerCase()) {
...
const updateXhr = (journalId, title, emotion, questionArr, ansArr) => {
  ...
  xhr.open("POST", "../../php/journal/updateJournal.php" ,true);
  xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
  xhr.onload = function(){
    ...
    if (this.status == 200) {
      ...
      if (res.success == "true"){
        location.href = "./journal.php?edit=success";
        ...
        location.href = "./journal.php?edit=error";
      }
...
const getUserInput = () => {
  ...
  updateXhr(journalId, title, emotion, questionArr, ansArr);
