...
<html lang="en" dir="ltr">
  <head>
    <meta charset="utf-8">
    <title>Request Reset Password email | griefSpace</title>
  ...
  <body class="fontInter">
    ...
    <?php
      if (!empty($_GET['status'])) {
        ...
        if ($status != "success") {
          ...
          if ($status == 'empty') {
            ...
            echo '<h2>No request:</h2>
                  <h3>No request made for this username.</h3>';
          } elseif ($status == 'error') {
            ...
            echo '<h2>Time limit:</h2>
                  <h3>You can submit a request every 30 minutes.</h3>';
          } elseif ($status == 'sendErr') {
            echo '<h2>Email error:</h2>
                  <h3>Oops... Email couldn\'t send out. Please contact out customer service.</h3>';
          }
          echo '</div>';
        }
        ...
        echo '<div class="message green flex alignCenter flexColumn">
                ...
                <h3>We\'ve sent an email to your email inbox. Please check your email inbox to proceed the procedure.</h3>
    ...
    <div class="formContainer flexColumn flex">
      <div class="wrapper loginWrapper">
        <form class="activationForm flex alignCenter flexColumn activeForm" action="../../php/loginReg/login/requestResetConn.php" method="post">
